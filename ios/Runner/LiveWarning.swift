...
/*: "USD" :*/
fileprivate let routerPresentData:[Character] = ["U","S","D"]
...
class LiveWarning: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = LiveWarning()
    ...
    func panelZone() {
        ...
        let adjustConfig = ADJConfig(appToken: kConnexionName, environment: environment)
        ...
        LiveWarning.thatPad(token: managerEnableceMsg)
    }
}
...
extension LiveWarning: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func readjustShow() -> String {
    ...
    class func thatPad(token: String) {
    ...
    class func collection(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (String(routerPresentData)))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func occurrence(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
