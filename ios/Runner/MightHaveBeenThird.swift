...
class MightHaveBeenThird {
    //: static let shared = AppPermissionTool()
    static let shared = MightHaveBeenThird()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func global(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func chase(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func push(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
